...
void Assign::_execute(Entity* entity) {
	...
	let = (*it);
        double value;
        if(let->getExpressionType() == DestinationType::VariableArray){
            Variable* auxvar = (Variable*) this->_model->getElementManager()->getElement(Util::TypeOf<Variable>(), let->getExpression());
            std::string indice = std::to_string(_model->parseExpression(let->getExpressionIndex()));
            value = auxvar->getValue(indice) + let->getC();
        }else{
            value = _model->parseExpression(let->getExpression()) + let->getC();
        }
	...
	if (let->getDestinationType() == DestinationType::Variable) {
	    ...
	    entity->setAttributeValue(let->getDestination(), value);
            ...
            Variable* myvar = (Variable*) this->_model->getElementManager()->getElement(Util::TypeOf<Variable>(), let->getDestination());
            std::string indice = std::to_string(_model->parseExpression(let->getDestinationIndex()));
